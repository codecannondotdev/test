<template>
	<Header :title="isEdit ? 'Edit Immunization' : 'Create Immunization'" />
	<ProgressBar
		v-if="loaders.size > 0"
		mode="indeterminate"
	...
	<Container class="edit">
		<Form
			...
			@deleted="router.push({ name: 'immunizations-list' })" />
		<VaccineRecordsRelationWidget
			v-if="isEdit"
			:immunization-id="route.params.id as string"
...
const isEdit = computed(() => route.name === 'immunizations-edit')
