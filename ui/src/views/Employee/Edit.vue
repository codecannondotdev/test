<template>
	<Header :title="isEdit ? 'Edit Employee' : 'Create Employee'" />
	<ProgressBar
		v-if="loaders.size > 0"
		mode="indeterminate"
	...
	<Container class="edit">
		<Form
			...
			@deleted="router.push({ name: 'employees-list' })" />
		<TransactionsRelationWidget
			v-if="isEdit"
			:employee-id="route.params.id as string"
			@start-loading="loaders.add('Transactions')"
			@stop-loading="loaders.delete('Transactions')" />
...
import TransactionsRelationWidget from './components/TransactionsRelationWidget.vue'
