<template>
	<Header :title="isEdit ? 'Edit Specialty' : 'Create Specialty'" />
	<ProgressBar
		v-if="loaders.size > 0"
		mode="indeterminate"
	...
	<Container class="edit">
		<Form
			...
			@deleted="router.push({ name: 'specialties-list' })" />
		<DoctorsRelationWidget
			v-if="isEdit"
			:specialty-id="route.params.id as string"
...
const isEdit = computed(() => route.name === 'specialties-edit')
