<template>
	<FormContainer
		...
		<form @submit.prevent="submit">
			...
			<FormInput
				v-if="!props.hideInputs?.includes('insurance_provider')"
				:required="false"
				:error-message="formErrors.insurance_provider"
				label="Insurance Provider">
				<InputText
					v-model="formData.insurance_provider"
					:disabled="!!props.forceValues.insurance_provider" />
			</FormInput>
			<FormInput
				v-if="!props.hideInputs?.includes('insurance_policy_number')"
				:required="false"
				:error-message="formErrors.insurance_policy_number"
				label="Insurance Policy Number">
				<InputText
					v-model="formData.insurance_policy_number"
					:disabled="!!props.forceValues.insurance_policy_number" />
			</FormInput>
			<FormInput
				v-if="!props.hideInputs?.includes('medical_history')"
				:required="false"
				:error-message="formErrors.medical_history"
				label="Medical History">
				<Textarea
					v-model="formData.medical_history"
					:disabled="!!props.forceValues.medical_history"
					rows="5"
					cols="50" />
			</FormInput>
			<FormInput
			...
			<FormInput
				v-if="!props.hideInputs?.includes('emergency_contact_phone')"
				:required="false"
				:error-message="formErrors.emergency_contact_phone"
				label="Emergency Contact Phone">
				<InputText
					v-model="formData.emergency_contact_phone"
					:disabled="!!props.forceValues.emergency_contact_phone" />
			</FormInput>
