<template>
	<FormContainer
		class="form"
		:visible
		:title="props.isEdit ? 'Update ResearchStudy' : 'Create ResearchStudy'"
		:as-dialog
		@close="emit('close')">
		<form @submit.prevent="submit">
			...
			<FormInput
				v-if="!props.hideInputs?.includes('principal_investigator')"
				:required="false"
				:error-message="formErrors.principal_investigator"
				label="Principal Investigator">
				<Textarea
					v-model="formData.principal_investigator"
					:disabled="!!props.forceValues.principal_investigator"
					rows="5"
					cols="50" />
			</FormInput>
			<FormInput
...
type FormData = {
	title: string
	principal_investigator: string | undefined
...
const { formData, loading, formErrors, reset, submit, remove } = useForm({
	...
	onCreated: (entity) => {
		if (props.shouldRedirect) {
			router.push({ name: 'research-studies-edit', params: { id: entity!.id } })
		}
		emit('created', entity)
	},
