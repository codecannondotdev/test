<template>
	<FormContainer
		class="form"
		:visible
		:title="props.isEdit ? 'Update Screening' : 'Create Screening'"
		:as-dialog
		@close="emit('close')">
		<form @submit.prevent="submit">
			...
			<FormInput
				v-if="!props.hideInputs?.includes('result')"
				:required="false"
				:error-message="formErrors.result"
				label="Result">
				<Textarea
					v-model="formData.result"
					:disabled="!!props.forceValues.result"
					rows="5"
					cols="50" />
			</FormInput>
			<FormInput
...
const { formData, loading, formErrors, reset, submit, remove } = useForm({
	...
	onCreated: (entity) => {
		if (props.shouldRedirect) {
			router.push({ name: 'screenings-edit', params: { id: entity!.id } })
		}
		emit('created', entity)
	},
