<template>
	<div class="header-toolbar">
		<Button
			...
			severity="secondary"
			@click="router.push({ name: 'physical-therapy-sessions-list' })" />
		<h2>{{ props.title }}</h2>
		<InputText
		...
		<Button
			...
			icon="fal fa-plus"
			label="Create"
			@click="router.push({ name: 'physical-therapy-sessions-create' })" />
	</div>
</template>
...
const isList = computed(() => route.name === 'physical-therapy-sessions-list')
