<template>
	<Header :title="isEdit ? 'Edit GeoLocation' : 'Create GeoLocation'" />
	<ProgressBar
		v-if="loaders.size > 0"
		mode="indeterminate"
	...
	<Container class="edit">
		<Form
			...
			@deleted="router.push({ name: 'geo-locations-list' })" />
		<HomeCareVisitsRelationWidget
			v-if="isEdit"
			:geo-location-id="route.params.id as string"
...
const isEdit = computed(() => route.name === 'geo-locations-edit')
