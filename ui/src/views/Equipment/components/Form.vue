<template>
	<FormContainer
		...
		<form @submit.prevent="submit">
			...
			<FormInput
				v-if="!props.hideInputs?.includes('purchase_date')"
				:required="false"
				:error-message="formErrors.purchase_date"
				label="Purchase Date">
				<DatePicker
					:model-value="formData.purchase_date ? new Date(formData.purchase_date) : undefined"
					:disabled="!!props.forceValues.purchase_date"
					selectionMode="single"
					@update:model-value="formData.purchase_date = ($event as Date | null)?.toISOString()" />
			</FormInput>
			<FormInput
...
const { formData, loading, formErrors, reset, submit, remove } = useForm({
	...
	onCreated: (entity) => {
		if (props.shouldRedirect) {
			router.push({ name: 'equipments-edit', params: { id: entity!.id } })
		}
		emit('created', entity)
	},
