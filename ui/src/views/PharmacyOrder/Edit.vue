<template>
	<Header :title="isEdit ? 'Edit PharmacyOrder' : 'Create PharmacyOrder'" />
	<ProgressBar
		v-if="loaders.size > 0"
		mode="indeterminate"
	...
	<Container class="edit">
		<Form
			...
			@deleted="router.push({ name: 'pharmacy-orders-list' })" />
		<MedicationsRelationWidget
			v-if="isEdit"
			:pharmacy-order-id="route.params.id as string"
...
const isEdit = computed(() => route.name === 'pharmacy-orders-edit')
