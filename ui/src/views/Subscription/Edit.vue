<template>
	<Header :title="isEdit ? 'Edit Subscription' : 'Create Subscription'" />
	<ProgressBar
		v-if="loaders.size > 0"
		mode="indeterminate"
	...
	<Container class="edit">
		<Form
			...
			@deleted="router.push({ name: 'subscriptions-list' })" />
		<PaymentsRelationWidget
			v-if="isEdit"
			:subscription-id="route.params.id as string"
...
const isEdit = computed(() => route.name === 'subscriptions-edit')
