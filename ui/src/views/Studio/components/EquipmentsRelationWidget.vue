<template>
	<Card class="equipments-relation-widget">
		<template #title>
			<h4 class="equipments-relation-widget__title">Equipments</h4>
			<Button
				icon="fal fa-link"
				label="Connect"
				outlined
...
import EquipmentsRelationAddDialog from './EquipmentsRelationAddDialog.vue'
...
function openDetails(item: { data: Equipment }) {
	router.push({ name: 'equipments-edit', params: { id: item.data.id } })
}
</script>

<style lang="scss" scoped>
.equipments-relation-widget {
	width: 100%;
	max-width: 600px;
	overflow: hidden;
	...
	:deep(.p-card-body) {
		...
		.p-card-caption {
			...
			.p-card-title {
				...
				.equipments-relation-widget__title {
					flex: 1;
					text-align: left;
				}
			}
