<template>
	<Card class="materials-relation-widget">
		<template #title>
			<h4 class="materials-relation-widget__title">Materials</h4>
			<Button
				icon="fal fa-link"
				label="Connect"
				outlined
	...
	<MaterialForm
		...
		:attach-to="{ suppliers: { method: 'syncWithoutDetaching', id: props.supplierId } }"
...
import MaterialsRelationAddDialog from './MaterialsRelationAddDialog.vue'
import MaterialForm from '@/views/Material/components/Form.vue'
...
async function detach(item: Material) {
	...
	:deep(.p-card-body) {
		...
		.p-card-caption {
			...
			.p-card-title {
				...
				.materials-relation-widget__title {
					flex: 1;
					text-align: left;
				}
			}
