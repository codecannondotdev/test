<template>
	<Card class="reported-expenses-relation-widget">
		<template #title>
			<h4 class="reported-expenses-relation-widget__title">Reported Expenses</h4>
			<Button
				icon="fal fa-link"
				label="Connect"
				outlined
	...
	<ReportedExpensesRelationAddDialog
	...
	<ExpenseForm
		...
		:attach-to="{ expenseReports: { method: 'syncWithoutDetaching', id: props.expenseReportId } }"
...
import ReportedExpensesRelationAddDialog from './ReportedExpensesRelationAddDialog.vue'
...
.reported-expenses-relation-widget {
	width: 100%;
	max-width: 600px;
	overflow: hidden;
	...
	:deep(.p-card-body) {
		...
		.p-card-caption {
			...
			.p-card-title {
				...
				.reported-expenses-relation-widget__title {
					flex: 1;
					text-align: left;
				}
			}
