<template>
	<Card class="claim-adjustments-relation-widget">
		<template #title>
			<h4 class="claim-adjustments-relation-widget__title">Claim Adjustments</h4>
			<Button
				icon="fal fa-link"
				label="Connect"
				outlined
				severity="secondary"
				@click="isRelationAddDialogActive = true" />
			<Button
				icon="fal fa-plus"
				label="Create"
				outlined
				severity="secondary"
				@click="isFormActive = true" />
		</template>
		<template #content>
			<ApiTable
				flat
				:list-state="listState"
				@row-click="openDetails">
				<Column
					field="adjustment_code"
					header="Adjustment Code" />
				<Column
					header=""
					:style="{ maxWidth: '72px', width: '72px' }">
					<template #body="slotProps">
						<Button
							severity="secondary"
							outlined
							rounded
							icon="fal fa-times"
							:loading="dissociateLoading === slotProps.data.id"
							@click.stop.prevent="dissociate(slotProps.data)" />
					</template>
				</Column>
			</ApiTable>
		</template>
	</Card>
	<ClaimAdjustmentsRelationAddDialog
		v-model="isRelationAddDialogActive"
		:insurance-claim-id="props.insuranceClaimId"
		@update="refresh()" />
	<ClaimAdjustmentForm
		:as-dialog="true"
		:visible="isFormActive"
		:should-redirect="false"
		:force-values="{ insurance_claim_id: Number(props.insuranceClaimId) }"
		:hide-inputs="['insurance_claim_id']"
		@close="isFormActive = false"
		@created="refresh()" />
</template>

<script setup lang="ts">
import { defineProps, onBeforeMount, ref } from 'vue'
import ClaimAdjustmentsRelationAddDialog from './ClaimAdjustmentsRelationAddDialog.vue'
import ClaimAdjustmentForm from '@/views/ClaimAdjustment/components/Form.vue'
import Card from 'primevue/card'
import ApiTable from '@/components/Table/ApiTable.vue'
import Column from 'primevue/column'
import Button from 'primevue/button'
import { useRouter } from 'vue-router'
import { useClaimAdjustmentListState } from '@/models/ClaimAdjustment/States'
import type { ClaimAdjustment } from '@/models/ClaimAdjustment/Model'
import type { InsuranceClaim } from '@/models/InsuranceClaim/Model'
import InsuranceClaimsApi from '@/models/InsuranceClaim/Api'

const emit = defineEmits(['start-loading', 'stop-loading'])

const props = defineProps<{
	insuranceClaimId: InsuranceClaim['id']
}>()

const router = useRouter()
const isRelationAddDialogActive = ref(false)
const isFormActive = ref(false)
const dissociateLoading = ref(null as null | number | string)
const listLoading = ref(false)
const listState = useClaimAdjustmentListState()
listState.defaultParams = {
	per_page: 10,
	fromRelation: {
		model: 'App\\Models\\InsuranceClaim',
		id: props.insuranceClaimId,
		relation: 'claimAdjustments',
	},
}

onBeforeMount(() => {
	refresh()
})

async function refresh() {
	listLoading.value = true
	emit('start-loading')
	try {
		await listState.getList()
	} finally {
		listLoading.value = false
		emit('stop-loading')
	}
}

async function dissociate(item: ClaimAdjustment) {
	dissociateLoading.value = item.id
	emit('start-loading')
	try {
		await new InsuranceClaimsApi().updateRelation(props.insuranceClaimId, 'claimAdjustments', {
			method: 'dissociate',
			params: [item.id],
		})
		dissociateLoading.value = null
		await refresh()
	} finally {
		dissociateLoading.value = null
		emit('stop-loading')
	}
}

function openDetails(item: { data: ClaimAdjustment }) {
	router.push({ name: 'claim-adjustments-edit', params: { id: item.data.id } })
}
</script>

<style lang="scss" scoped>
.claim-adjustments-relation-widget {
	width: 100%;
	max-width: 600px;
	overflow: hidden;

	:deep(.p-card-body) {
		padding: 20px 0 0;

		.p-card-caption {
			padding: 0px 20px 12px;

			.p-card-title {
				display: flex;
				align-items: center;
				gap: 10px;

				.claim-adjustments-relation-widget__title {
					flex: 1;
					text-align: left;
				}
			}
		}
	}
}
</style>
