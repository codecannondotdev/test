<template>
	<FormContainer
		class="form"
		:visible
		:title="props.isEdit ? 'Update Instructor' : 'Create Instructor'"
		:as-dialog
		@close="emit('close')">
		<form @submit.prevent="submit">
			...
			<FormInput
				v-if="!props.hideInputs?.includes('bio')"
				:required="false"
				:error-message="formErrors.bio"
				label="Bio">
				<Textarea
					v-model="formData.bio"
					:disabled="!!props.forceValues.bio"
					rows="5"
					cols="50" />
			</FormInput>
			<FormInput
...
const { formData, loading, formErrors, reset, submit, remove } = useForm({
	...
	onCreated: (entity) => {
		if (props.shouldRedirect) {
			router.push({ name: 'instructors-edit', params: { id: entity!.id } })
		}
		emit('created', entity)
	},
