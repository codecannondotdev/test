<template>
	<Header :title="isEdit ? 'Edit ClinicalTrial' : 'Create ClinicalTrial'" />
	<ProgressBar
		v-if="loaders.size > 0"
		mode="indeterminate"
	...
	<Container class="edit">
		<Form
			...
			@deleted="router.push({ name: 'clinical-trials-list' })" />
		<PatientsRelationWidget
			v-if="isEdit"
			:clinical-trial-id="route.params.id as string"
...
const isEdit = computed(() => route.name === 'clinical-trials-edit')
