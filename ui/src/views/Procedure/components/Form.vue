<template>
	<FormContainer
		class="form"
		:visible
		:title="props.isEdit ? 'Update Procedure' : 'Create Procedure'"
		:as-dialog
		@close="emit('close')">
		<form @submit.prevent="submit">
			...
			<FormInput
				v-if="!props.hideInputs?.includes('treatment_id')"
				:required="false"
				:error-message="formErrors.treatment_id"
				label="Treatment">
				<ModelSelect
					v-model="formData.treatment_id"
					:api="new TreatmentsApi()"
					:disabled="!!props.forceValues.treatment_id"
					option-label="title" />
			</FormInput>
...
const { formData, loading, formErrors, reset, submit, remove } = useForm({
	...
	onCreated: (entity) => {
		if (props.shouldRedirect) {
			router.push({ name: 'procedures-edit', params: { id: entity!.id } })
		}
		emit('created', entity)
	},
