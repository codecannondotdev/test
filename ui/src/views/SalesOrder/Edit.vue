<template>
	<Header :title="isEdit ? 'Edit SalesOrder' : 'Create SalesOrder'" />
	<ProgressBar
		v-if="loaders.size > 0"
		mode="indeterminate"
	...
	<Container class="edit">
		<Form
			...
			@deleted="router.push({ name: 'sales-orders-list' })" />
		<ShipmentsRelationWidget
			v-if="isEdit"
			:sales-order-id="route.params.id as string"
...
const isEdit = computed(() => route.name === 'sales-orders-edit')
