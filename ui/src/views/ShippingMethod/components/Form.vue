<template>
	<FormContainer
		class="form"
		:visible
		:title="props.isEdit ? 'Update ShippingMethod' : 'Create ShippingMethod'"
		:as-dialog
		@close="emit('close')">
		<form @submit.prevent="submit">
			<FormInput
				v-if="!props.hideInputs?.includes('method_name')"
				:required="true"
				:error-message="formErrors.method_name"
				label="Method Name">
				<InputText
					v-model="formData.method_name"
					:disabled="!!props.forceValues.method_name" />
			</FormInput>
			<FormInput
...
const { formData, loading, formErrors, reset, submit, remove } = useForm({
	...
	onCreated: (entity) => {
		if (props.shouldRedirect) {
			router.push({ name: 'shipping-methods-edit', params: { id: entity!.id } })
		}
		emit('created', entity)
	},
