<template>
	<Header :title="isEdit ? 'Edit LegalHold' : 'Create LegalHold'" />
	<ProgressBar
		v-if="loaders.size > 0"
		mode="indeterminate"
	...
	<Container class="edit">
		<Form
			...
			@deleted="router.push({ name: 'legal-holds-list' })" />
		<DocumentsRelationWidget
			v-if="isEdit"
			:legal-hold-id="route.params.id as string"
...
const isEdit = computed(() => route.name === 'legal-holds-edit')
