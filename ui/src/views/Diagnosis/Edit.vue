<template>
	<Header :title="isEdit ? 'Edit Diagnosis' : 'Create Diagnosis'" />
	<ProgressBar
		v-if="loaders.size > 0"
		mode="indeterminate"
	...
	<Container class="edit">
		<Form
			...
			@deleted="router.push({ name: 'diagnoses-list' })" />
		<TreatmentsRelationWidget
			v-if="isEdit"
			:diagnosis-id="route.params.id as string"
...
const isEdit = computed(() => route.name === 'diagnoses-edit')
