<template>
	<FormContainer
		class="form"
		:visible
		:title="props.isEdit ? 'Update Settings' : 'Create Settings'"
		:as-dialog
		@close="emit('close')">
		<form @submit.prevent="submit">
			<FormInput
				v-if="!props.hideInputs?.includes('key')"
				:required="true"
				:error-message="formErrors.key"
				label="Key">
				<InputText
					v-model="formData.key"
					:disabled="!!props.forceValues.key" />
			</FormInput>
			<FormInput
...
const { formData, loading, formErrors, reset, submit, remove } = useForm({
	...
	onCreated: (entity) => {
		if (props.shouldRedirect) {
			router.push({ name: 'settings-edit', params: { id: entity!.id } })
		}
		emit('created', entity)
	},
