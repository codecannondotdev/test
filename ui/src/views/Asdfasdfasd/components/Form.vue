<template>
	<FormContainer
		class="form"
		:visible
		:title="props.isEdit ? 'Update Asdfasdfasd' : 'Create Asdfasdfasd'"
		:as-dialog
		@close="emit('close')">
		<form @submit.prevent="submit">
			...
			<FormInput
				...
				<DatePicker
					...
					@update:model-value="formData.date_of_birth = ($event as Date | null)?.toISOString()" />
			</FormInput>
			<FormInput
				v-if="!props.hideInputs?.includes('boolean')"
				:error-message="formErrors.boolean"
				label="Boolean">
				<template #prepend-input>
					<Checkbox
						v-model="formData.boolean"
						:disabled="!!props.forceValues.boolean"
						:binary="true" />
				</template>
			</FormInput>
...
const { formData, loading, formErrors, reset, submit, remove } = useForm({
	...
	onCreated: (entity) => {
		if (props.shouldRedirect) {
			router.push({ name: 'asdfasdfasds-edit', params: { id: entity!.id } })
		}
		emit('created', entity)
	},
