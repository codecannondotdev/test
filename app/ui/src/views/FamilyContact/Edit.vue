<template>
	<Header :title="isEdit ? 'Edit FamilyContact' : 'Create FamilyContact'" />
	<ProgressBar
		v-if="loaders.size > 0"
		mode="indeterminate"
	...
	<Container class="edit">
		<Form
			...
			@deleted="router.push({ name: 'family-contacts-list' })" />
		<NotificationsRelationWidget
			v-if="isEdit"
			:family-contact-id="route.params.id as string"
...
const isEdit = computed(() => route.name === 'family-contacts-edit')
