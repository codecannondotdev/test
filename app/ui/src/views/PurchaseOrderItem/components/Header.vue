<template>
	<div class="header-toolbar">
		<Button
			...
			severity="secondary"
			@click="router.push({ name: 'purchase-order-items-list' })" />
		<h2>{{ props.title }}</h2>
		<InputText
		...
		<Button
			...
			icon="fal fa-plus"
			label="Create"
			@click="router.push({ name: 'purchase-order-items-create' })" />
	</div>
</template>
...
const isList = computed(() => route.name === 'purchase-order-items-list')
