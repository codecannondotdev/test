<template>
	<Header :title="isEdit ? 'Edit Procedure' : 'Create Procedure'" />
	<ProgressBar
		v-if="loaders.size > 0"
		mode="indeterminate"
	...
	<Container class="edit">
		<Form
			...
			@deleted="router.push({ name: 'procedures-list' })" />
		<SurgeriesRelationWidget
			v-if="isEdit"
			:procedure-id="route.params.id as string"
...
const isEdit = computed(() => route.name === 'procedures-edit')
