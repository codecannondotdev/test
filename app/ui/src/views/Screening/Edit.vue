<template>
	<Header :title="isEdit ? 'Edit Screening' : 'Create Screening'" />
	<ProgressBar
		v-if="loaders.size > 0"
		mode="indeterminate"
	...
	<Container class="edit">
		<Form
			...
			@deleted="router.push({ name: 'screenings-list' })" />
		<FollowUpRelationWidget
			v-if="isEdit"
			:screening-id="route.params.id as string"
...
const isEdit = computed(() => route.name === 'screenings-edit')
