<template>
	<Header :title="isEdit ? 'Edit Shipment' : 'Create Shipment'" />
	<ProgressBar
		v-if="loaders.size > 0"
		mode="indeterminate"
	...
	<Container class="edit">
		<Form
			...
			@deleted="router.push({ name: 'shipments-list' })" />
		<ShipmentItemsRelationWidget
			v-if="isEdit"
			:shipment-id="route.params.id as string"
...
const isEdit = computed(() => route.name === 'shipments-edit')
