<template>
	<Header :title="isEdit ? 'Edit AppointmentType' : 'Create AppointmentType'" />
	<ProgressBar
		v-if="loaders.size > 0"
		mode="indeterminate"
	...
	<Container class="edit">
		<Form
			...
			@deleted="router.push({ name: 'appointment-types-list' })" />
		<AppointmentsRelationWidget
			v-if="isEdit"
			:appointment-type-id="route.params.id as string"
...
const isEdit = computed(() => route.name === 'appointment-types-edit')
