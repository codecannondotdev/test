<template>
	<Header :title="isEdit ? 'Edit NutritionalPlan' : 'Create NutritionalPlan'" />
	<ProgressBar
		v-if="loaders.size > 0"
		mode="indeterminate"
	...
	<Container class="edit">
		<Form
			...
			@deleted="router.push({ name: 'nutritional-plans-list' })" />
		<DietOrdersRelationWidget
			v-if="isEdit"
			:nutritional-plan-id="route.params.id as string"
...
const isEdit = computed(() => route.name === 'nutritional-plans-edit')
