<template>
	<Header :title="isEdit ? 'Edit ReferralSource' : 'Create ReferralSource'" />
	<ProgressBar
		v-if="loaders.size > 0"
		mode="indeterminate"
	...
	<Container class="edit">
		<Form
			...
			@deleted="router.push({ name: 'referral-sources-list' })" />
		<ReferralsRelationWidget
			v-if="isEdit"
			:referral-source-id="route.params.id as string"
...
const isEdit = computed(() => route.name === 'referral-sources-edit')
