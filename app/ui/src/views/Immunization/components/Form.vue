<template>
	<FormContainer
		class="form"
		:visible
		:title="props.isEdit ? 'Update Immunization' : 'Create Immunization'"
		:as-dialog
		@close="emit('close')">
		<form @submit.prevent="submit">
			...
			<FormInput
				v-if="!props.hideInputs?.includes('schedule')"
				:required="false"
				:error-message="formErrors.schedule"
				label="Schedule">
				<Textarea
					v-model="formData.schedule"
					:disabled="!!props.forceValues.schedule"
					rows="5"
					cols="50" />
			</FormInput>
...
const { formData, loading, formErrors, reset, submit, remove } = useForm({
	...
	onCreated: (entity) => {
		if (props.shouldRedirect) {
			router.push({ name: 'immunizations-edit', params: { id: entity!.id } })
		}
		emit('created', entity)
	},
