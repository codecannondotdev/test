<template>
	<Card class="referrals-received-relation-widget">
		<template #title>
			<h4 class="referrals-received-relation-widget__title">Referrals Received</h4>
			<Button
				icon="fal fa-link"
				label="Connect"
				outlined
	...
	<ReferralsReceivedRelationAddDialog
	...
	<ReferralForm
		...
		:force-values="{ receiving_provider_id: Number(props.doctorId) }"
		:hide-inputs="['receiving_provider_id']"
...
import ReferralsReceivedRelationAddDialog from './ReferralsReceivedRelationAddDialog.vue'
...
.referrals-received-relation-widget {
	width: 100%;
	max-width: 600px;
	overflow: hidden;
	...
	:deep(.p-card-body) {
		...
		.p-card-caption {
			...
			.p-card-title {
				...
				.referrals-received-relation-widget__title {
					flex: 1;
					text-align: left;
				}
			}
